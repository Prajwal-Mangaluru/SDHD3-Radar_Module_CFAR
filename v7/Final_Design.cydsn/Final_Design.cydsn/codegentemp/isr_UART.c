...
void isr_UART_SetVector(cyisraddress address)
{
    ...
    ramVectorTable[CYINT_IRQ_BASE + (uint32)isr_UART__INTC_NUMBER] = address;
}
...
cyisraddress isr_UART_GetVector(void)
{
    ...
    return ramVectorTable[CYINT_IRQ_BASE + (uint32)isr_UART__INTC_NUMBER];
}
...
*
*   Note calling isr_UART_Start or isr_UART_StartEx will 
*   override any effect this API would have had. This API should only be called
*   after isr_UART_Start or isr_UART_StartEx has been called. 
...
{
    *isr_UART_INTC_PRIOR = priority << 5;
}
...
uint8 isr_UART_GetPriority(void)
{
    ...
    priority = *isr_UART_INTC_PRIOR >> 5;
